...
void select201817_EEPrefire(const TString conf="samples.conf", // input file
  ...
  std::vector<float> *jetAK8_puppi_jer_sf_down = new std::vector<float>();
	//--Jets(AK4)--
  Int_t jetAK4_N = -99;
  std::vector<float> *jetAK4_pt = new std::vector<float>();
  std::vector<float> *jetAK4_eta = new std::vector<float>();
  //--Trigger
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
...
#else
	TString outfilename1 = ntupDir + TString("/") + snamev[isam] + TString("EEPrefire_nominal_pileup_WGamma_full_full_Apr29.root");
#endif
	...
	for(UInt_t ifile=0; ifile<nfiles; ifile++) {
		...
		//--Jets (AK4 PUPPI)
		eventTree->SetBranchAddress("jetAK4_N", &jetAK4_N);                                  
		eventTree->SetBranchAddress("jetAK4_pt", &jetAK4_pt);             
		eventTree->SetBranchAddress("jetAK4_eta", &jetAK4_eta);            
	...
	// for(UInt_t ientry=0; ientry<5; ientry++) {
	count0++;
	...
	*/
	...
	//-----------------------------------------EE pre-firing exclusion--------------------------------------------------
	bool pass_prefire = false;
	int N_AK4_prefire = 0;
	for(int i=0; i<jetAK4_pt->size(); i++){
		if(jetAK4_pt->at(i) < 100) continue;
		if(abs(jetAK4_eta->at(i)) < 2.25 || abs(jetAK4_eta->at(i)) > 3.0) continue;
		N_AK4_prefire++;
	}
	if(N_AK4_prefire==0){
		pass_prefire = true;
	...
	*/
	if(pass_p2 && pass_j2 && pass_s1 && pass_prefire){
#if runmode == 11
	...
	*/
	...
	cout<<"Rejected due to EE-prefire"<<count_rej_EEprefire<<endl;
	outFile1->cd();
	histpileup->Write();
    outFile1->Write();
    outFile1->Close();
