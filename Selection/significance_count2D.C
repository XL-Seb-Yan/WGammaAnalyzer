...
void significance_count2D(const TString conf="samples.conf"){
  ...
  */
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
    ...
    for(UInt_t ifile=0; ifile<nfiles; ifile++) {  
      ...
      if(isam == 0){
	for(int i = 10; i*0.1 < 3; i++){
	  std::vector<int> sig_N;
	  for(int j = 4; j*0.05 < 0.5; j++){
	    int N = 0;
	    for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
	      ...
	      //if(abs(photon_eta) < i*0.1 && abs(ak8puppijet_eta) < j*0.1 && (sys_invmass > 1190 && sys_invmass < 1610)){
	      //if(abs(photon_eta) < i*0.1 && sys_costhetastar < j*0.05 && (sys_invmass > 510 && sys_invmass < 690)){
	      //if(sys_ptoverm > i*0.05 && sys_costhetastar < j*0.05 && (sys_invmass > 510 && sys_invmass < 690)){
	      //if(abs(ak8puppijet_masssoftdropcorr - 80.38) < i*1 && ak8puppijet_tau21 < j*0.05 && (sys_invmass > 1190 && sys_invmass < 1610)){
	      //if(abs(ak8puppijet_eta) < i*0.1 && ak8puppijet_tau21 < j*0.05 && (sys_invmass > 510 && sys_invmass < 690)){
	      //if(abs(ak8puppijet_eta) < i*0.1 && abs(ak8puppijet_masssoftdropcorr - 80.38) < j*1 && (sys_invmass > 1190 && sys_invmass < 1610)){
	      if(abs(photon_eta) < i*0.1 && sys_ptoverm > j*0.05 && (sys_invmass > 510 && sys_invmass < 690)){
		N++;
	      }
	    }
	    sig_N.push_back(N);
  ...
  //TString Graphname ="|#eta| cos(#theta*) cut correlation (photon)";
  //gr->SetTitle(Graphname+"; |#eta| cut photon; cos(#theta*) cut photon");
  //TString Graphname ="pt/M cos(#theta*) cut correlation (photon)";
  //gr->SetTitle(Graphname+"; pt/M cut photon; cos(#theta*) cut photon");
  // TString Graphname ="mass tau21 cut correlation (jet)";
  //gr->SetTitle(Graphname+"; mass window cut jet; tau21 cut jet");
  //TString Graphname ="|#eta| tau21 cut correlation (jet)";
  //gr->SetTitle(Graphname+"; |#eta| cut jet; tau21 cut jet");
  //TString Graphname ="mass |#eta| cut correlation (jet)";
  //gr->SetTitle(Graphname+"; |#eta| cut jet; mass window cut jet");
  TString Graphname ="|#eta| pt/M cut correlation (photon)";
  gr->SetTitle(Graphname+"; |#eta| cut photon; pt/M cut photon");
  ...
  for(int i=0; i<sig_N2D.size(); i++){
    for(int j=0; j<sig_N2D[0].size(); j++){
      if(bkg_N2D[i].at(j) < 0.00001 || sig_N2D[i].at(j) < 0.00001){
	continue;
      }
      else{
	cout<<recordarr1[i]*0.1<<" "<<recordarr2[j]*1<<" "<<float(sig_N2D[i].at(j))/float(sqrt(bkg_N2D[i].at(j)))<<endl;
	gr->SetPoint(point,recordarr1[i]*0.1,recordarr2[j]*0.05,float(sig_N2D[i].at(j))/float(sqrt(bkg_N2D[i].at(j))));
	point++;
      }
    }
  }
