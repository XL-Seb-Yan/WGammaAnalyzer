#include <TMath.h>
#include <TLegend.h>
void make_DsPhiPi_shapes(int seed=37)
{
  ...
  // --- Create workspace --- 
  RooWorkspace w("w","w");
  RooRealVar sysinvmass("sysinvmass","invmass",1.6,2.3,""); //the name "sysinvmass" will be used by RooDataSet to import data
  sysinvmass.setBins(70);

  //--- signal PDF ---
  RooRealVar* mu1 = new RooRealVar("DG_mu1","#mu1",1.96847,1.9,2.0,"");
  ...
  TFile file("/afs/cern.ch/user/x/xuyan/3MuonProj/CMSSW_8_0_27/src/MCFlat/ntuples/dstau_Phipi_select.root");
  ...
  data.plotOn(frame);
  frame->SetTitle("m_{#mu^{+}#mu^{-}#pi^{#mp}} MC");
  ...
  xaxis->SetTitle("m_{#mu^{+}#mu^{-}#pi^{#mp}} (GeV)");
  ...
  wUP->var("sysinvmass[1.6,2.3]");
  wUP->import(data,Rename("MC"));
  wUP->import(*doublegauss);
  wUP->import(*bern);
  wUP->writeToFile("DsPhiPi-shapes-UnbinnedParam.root");
}
