...
void select_trigger1718(const TString conf="samples.conf", // input file
  ...
  TH1F* hist03lpa = new TH1F("WGamma03lpa",histotitle+", pt_{#gamma}",100,0,2000);//Fired 2 HLT, pt turn on
  ...
  TH1F* hist03hpa = new TH1F("WGamma03hpa",histotitle+", pt_{#gamma}",100,0,2000);//Fired all HLT, mass turn on
  ...
  TFile *outFile = TFile::Open("Trigger_"+era+".root", "RECREATE");
  hist00lpa->Write();
  hist00lpb->Write();
  hist00lpc->Write();
  hist00hpa->Write();
