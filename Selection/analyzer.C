  ...
  */
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
    ...
    for(UInt_t ifile=0; ifile<nfiles; ifile++) {  
      ...
      if(isam == 0){
	for(float cut1 = 0; cut1 < 2200; cut1+=20){
	  recordarr1.push_back(cut1);
	  std::vector<int> sig_N;
	  for(float cut2 = 0; cut2 < 2200; cut2+=20){
	    recordarr2.push_back(cut2);
	    int N = 0;
	    for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
	      ...
	      if(photon_pt > cut1 && ak8puppijet_pt > cut2 && (sys_invmass > 1360 && sys_invmass < 1840)){
		N++;
	      }
	    }
	    if(N == 0)
  ...
  for(int i=0; i<sig_N2D.size(); i++){
	...
	gr->SetPoint(i*sig_N2D.size()+j,recordarr1[i],recordarr2[j],float(sig_N2D[i].at(j))/float(sqrt(bkg_N2D[i].at(j))));
      }
    }
  }
