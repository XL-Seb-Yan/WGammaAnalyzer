#include <TMath.h>
#include <TLegend.h>
void massfit_sig(int seed=37)
{
  using namespace RooFit;
  RooRandom::randomGenerator()->SetSeed(37); 

  // --- Create Variable --- 
  RooRealVar mass("mass","mass",1.6,2,""); //the name "BDT_mass" will be used by RooDataSet to import data
  mass.setBins(40);

  //--- signal PDF ---
  RooRealVar* mu1 = new RooRealVar("DG_mu1","#mu1",1.77682,1.6,1.9,"");
  RooRealVar* mu2 = new RooRealVar("DG_mu2","#mu2",1.77682,1.6,1.9,"");
  RooRealVar* sigma1 = new RooRealVar("DG_sigma1","#sigma_{1}",0.025,0.01,0.06,"");
  RooRealVar* sigma2 = new RooRealVar("DG_sigma2","#sigma_{2}",0.015,0.01,0.02,"");
  ...
  RooBernstein* bern = new RooBernstein("Bern","",mass,RooArgList(*pC,*p0,*p1));
  ...
  TFile file("/afs/cern.ch/work/x/xuyan/work4/CMSSW_8_0_27/src/Tau3Mu_Ana/TMVA/InvMassS3.root");
  ...
  //mass.setRange("SB1",1.47682,1.67682);
  //mass.setRange("SB2",1.87682,2.07682);
  ...
  frame->SetTitle("Signal MC");
  //ex_sum->plotOn(frame,LineColor(kRed));
  doublegauss->plotOn(frame,LineColor(kRed),RooFit::Name("sigfun"));
  ...
  doublegauss->plotOn(frame,VisualizeError(*r,1),FillColor(kOrange),RooFit::Name("err"));
  ...
  // residuals
  RooHist* hresid = frame->residHist("data","sigfun");
  ...
  xaxis2->SetTitle("m_{#mu^{#mp}#mu^{#pm}#mu^{#mp}} (GeV)");
