...
void select(const TString conf="samples.conf", // input file
  ...
  */
  ...
  TH1F* hist05ja = new TH1F("WGamma05ja",histotitle+", m_{j} (AK8 Puppi Softdrop Corrected)",50,60,110);
  ...
  TH1F* hist05jb = new TH1F("WGamma05jb",histotitle+", m_{j} (AK8 Puppi Softdrop Corrected)",50,60,110);
...
SetLineStyle(2); histMC06a->Scale(scale);
  ...
  TF1 *g2 = new TF1 ("m2", "gaus", 65, 105);
  g2->SetLineColor(kGreen);
  TF1 *f1 = new TF1("double_gaus", "gaus(0) + gaus(3)", 65, 105);
  f1->SetParNames("Constant 1", "Mean 1", "Sigma 1","Constant 2", "Mean 2", "Sigma 2");
  f1->SetLineColor(kBlue);  
  gStyle->SetOptFit(1); 
  Double_t par1[6]= {70,78,7,10,85,20};
  f1->SetParameters(par1);
  //hist05jb->Fit(f1, "R+");
  g1->SetParameters(115.6, 80.2, 6.13);
  g2->SetParameters(61.9, 79.3, 11.1);

  TF1 *g3 = new TF1 ("m1", "gaus", 200, 2200);
  g1->SetLineColor(kRed);
  TF1 *g4 = new TF1 ("m2", "expo", 200, 2200);
  g2->SetLineColor(kGreen);
  TF1 *f2 = new TF1("gausexpo", "gaus(0) + expo(3)", 200, 2200);
  f2->SetParNames("Constant 1", "Mean 1", "Sigma 1","Constant 2", "exponent 2");
  f2->SetLineColor(kBlue);  
  gStyle->SetOptFit(1); 
  Double_t par2[6]= {700,1570,20,10,-3};
  f2->SetParameters(par2);
  //hist01sb->Fit(f2, "R+");
  //g3->SetParameters(115.6, 80.2, 6.13);
  //g4->SetParameters(61.9, 79.3, 11.1);
