...
void make_sideband_shapes(int seed=37)
{
  ...
  */
  ...
  RooFitResult *r = NULL;
  if(isNorm)
    r = model->fitTo(data_norm,Range(600,3000),RooFit::Minimizer("Minuit2"),SumW2Error(false),Save()); //SumW2Error(false) for weighted data, see how to choose this with same calling without SumW2Error(false)
  else
    r = model->fitTo(data,Range(600,3000),RooFit::Minimizer("Minuit2"),Save()); //SumW2Error(false) for weighted data, see how to choose this with same calling without SumW2Error(false)
  ...
  if(isNorm){
    ...
    data_norm.plotOn(frame,RooFit::Name("data"),Binning(95,600,2500),DataError(RooAbsData::SumW2)); //for weighted data
  }
  else{
    frame->SetTitle("Data Sideband");
    data.plotOn(frame,RooFit::Name("data"),Binning(95,600,2500),DataError(RooAbsData::Poisson)); //for unweighted data
  }
  ...
  model->plotOn(frame,LineStyle(kDashed),RooFit::Name(fun_name));
  if(isNorm)
    data_norm.plotOn(frame,RooFit::Name("data"),Binning(120,600,3000),DataError(RooAbsData::SumW2)); //for weighted data
  else
    data.plotOn(frame,RooFit::Name("data"),Binning(120,600,3000),DataError(RooAbsData::Poisson)); //for unweighted data
  ...
  //RooChi2Var chi2 ("chi2", "chi2", *model,datah,DataError(RooAbsData::SumW2));
  ...
  //RooAbsReal *chi2Sum = model->createChi2(datah,DataError(RooAbsData::SumW2));
  cout<<"chi2 with possion is: "<<chi2.getVal()<<endl;
  cout<<"chi2 with expected is: "<<chi2Exp->getVal()<<endl;
  //cout<<"chi2 with SumW2 is: "<<chi2Sum->getVal()<<endl;
  ...
  */
  ...
  TLatex *NLLlax = new TLatex(1830,180,NLLtxt);
  ...
  if(isNorm)
    ...
    ex_r = ex_model->fitTo(data_norm,Range(600,3000),RooFit::Minimizer("Minuit2"),Extended(true),Save());
