...
void make_signalPDF_W_CB2Gaus()
{
  ...
  for(int i=1800; i<3501; i+=100){
	...
	// --- Plot ---
	  ...
	  com_model->plotOn(frame,Components("CB"),LineStyle(kDashed),LineColor(1),RooFit::Name("CB"),Range(i*0.75,i*1.25));
	  com_model->plotOn(frame,Components("Gaussian_1"),LineStyle(kDashed),LineColor(30),RooFit::Name("Gaus1"),Range(i*0.75,i*1.25));
	  com_model->plotOn(frame,Components("Gaussian_2"),LineStyle(kDashed),LineColor(42),RooFit::Name("Gaus2"),Range(i*0.75,i*1.25));
	  com_model->plotOn(frame,LineColor(2),RooFit::Name("fit"),Range(i*0.75,i*1.25));
	  datah.plotOn(frame);
	  frame->getAttMarker()->SetMarkerSize(2);
	  ...
	  pull_frame->Draw();
	  p01b->Update();
	  TString plotname = "CB2Gaus"+mass_str+".png";
	  c01->Print(plotname);
