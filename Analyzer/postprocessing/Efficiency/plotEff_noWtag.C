...
void plotEff_noWtag(){
  ...
  TLegend *leg = new TLegend(0.5,0.65,0.85,0.75);
  leg->SetBorderSize(0);
  leg->SetNColumns(2);
  leg->SetFillStyle(0);
  ...
  yaxis->SetTitle("A #varepsilon (w/o W tag)");
  ...
  yaxis->SetTitle("#varepsilon w/o W tag");
  yaxis->SetTitleOffset(1.2);
  ...
  CMS_lumi(c3,5,iPos);
  leg->Clear();
  leg->AddEntry(gr9,"Narrow S-0","lep");
  leg->AddEntry(gr10,"Broad S-0","lep");
  leg->AddEntry(gr13,"Narrow S-1","lep");
  leg->AddEntry(gr14,"Broad S-1","lep");
  leg->Draw();
  c3->SetGrid();
  c3->Print("Eff17.png");
  c3->Print("Eff17.pdf");
